...
if torch is not None:
    from autoencoders import build_mlp_backbone_kwargs_from_model_config, DIPVariationalAutoencoderConfig, DIPVariationalAutoencoderModel
...
class DIPVariationalAutoencoderModelTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.config = DIPVariationalAutoencoderConfig(input_dim=16, latent_dim=4, hidden_dims=[12, 8], dip_weight=10.0)

    def test_forward_returns_dip_loss(self) -> None:
        model = DIPVariationalAutoencoderModel(config=self.config, **build_mlp_backbone_kwargs_from_model_config(self.config))
