...
if torch is not None:
    ...
    from autoencoders import BetaTCVariationalAutoencoderConfig, BetaTCVariationalAutoencoderModel
...
class BetaTCVariationalAutoencoderModelTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.config = BetaTCVariationalAutoencoderConfig(latent_dim=4, hidden_dims=[12, 8])

    def test_forward_returns_decomposed_kl_terms(self) -> None:
        model = BetaTCVariationalAutoencoderModel(config=self.config, **build_mlp_backbone_kwargs_from_model_config(self.config, feature_dim=16))
