...
class VariationalAutoencoderModelTest(unittest.TestCase):
    ...
    def test_forward_returns_expected_fields(self) -> None:
        model = VariationalAutoencoderModel(config=self.config, **build_mlp_backbone_kwargs_from_model_config(self.config, feature_dim=16))
        model.train()
    ...
    def test_training_forward_requires_current_epoch_when_warmup_is_enabled(self) -> None:
        ...
        model = VariationalAutoencoderModel(config=config, **build_mlp_backbone_kwargs_from_model_config(config, feature_dim=16))
        model.train()

        with self.assertRaisesRegex(ValueError, "current_epoch"):
            model(inputs=self.inputs)

    def test_auto_decoder_is_rejected_when_decoder_input_spec_differs_from_encoder_output(self) -> None:
